...
export default function FlatmateDetailPage() {
  ...
  const headerActions = isOwner ? (
    <>
      ...
      {listing.status === "open" && (
          <button onClick={() => handleStatusUpdate("closed")} className="text-sm font-medium text-ruin-background bg-[#00C9A7] px-3 py-1 rounded-md hover:bg-teal-500 transition-colors">Mark as Closed</button>
      )}
  ...
  const mainContent = (
    <>
        <p className="-mt-4 mb-4 text-lg text-ruin-muted flex items-center gap-2">
  ...
  const bottomContent = isOwner && listing.inquiries ? (
    <InquiriesList 
