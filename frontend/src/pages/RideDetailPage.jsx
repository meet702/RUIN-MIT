...
export default function RideDetailPage() {
    ...
    <>
      ...
      {(ride.status === "open" || ride.status === "full") && (
          <>
              ...
              <button onClick={() => handleStatusUpdate("cancelled")} className="text-sm font-medium text-ruin-magenta px-3 py-1 border border-ruin-magenta rounded-md hover:bg-ruin-magenta/10 transition-colors">Cancel Ride</button>
          </>
      )}
  ...
  const mainContent = (
    <>
        <div className="flex flex-col gap-6 relative p-6 sm:p-8 rounded-2xl border border-ruin-border bg-ruin-card">
            <div className="absolute left-9 sm:left-11 top-14 bottom-14 w-0.5 bg-ruin-border hidden sm:block"></div>
  ...
  const sidebarContent = (
    <div className="rounded-2xl border border-ruin-border bg-ruin-background p-6 h-fit text-center">
        ...
        {!isOwner && (
            <>
                {isPassenger ? (
                    <div>
                        <div className="inline-flex items-center gap-2 py-2 px-4 rounded-full bg-[#00C9A7]/10 border border-[#00C9A7]/30 text-[#00C9A7] font-semibold text-sm mb-3">
                            <CircleDot size={14} /> You have booked a seat!
                        </div>
                        <button onClick={handleCancelBooking} className="block w-full text-sm font-medium text-ruin-magenta hover:underline">
                            Cancel Booking
                        </button>
                    </div>
                ) : ride.status === "open" ? (
                    ...
                    <div className="inline-flex items-center gap-2 py-2 px-4 rounded-full bg-ruin-card border border-ruin-border text-ruin-muted font-semibold text-sm">
        ...
        {isOwner && (
            <div className="inline-flex items-center gap-2 py-2 px-4 rounded-full bg-ruin-card border border-ruin-border text-ruin-text font-semibold text-sm">
                You are the driver
            </div>
        )}

        {!isOwner && isPassenger && (
            <div className="mt-6 pt-6 border-t border-ruin-border">
                <ChatButton
  ...
  const bottomContent = (isOwner || isPassenger) && ride.passengers && ride.passengers.length > 0 ? (
  ...
  return (
    <>
      {isDeleting && <ActionLoader message="Deleting ride..." />}
      <DetailPageLayout
        badges={badges}
